...
```jsonc
{
  ...
  "category": "composite",             // 专项分类（2026-09-03 加，必填，枚举见 §九；App「专项训练」按它筛选）
...
**查重双闸（2026-09-03 加，第②③步之间）**：同考点重复训练是判分题的浪费，
两道闸分工——`skeleton-check` 抓「换皮撞车」（骨架同＝铁撞，直接否决）；
`skeleton-check` 抓不住的**近亲**（骨架不同、练同一件事）由 `nearcheck` 补：
作者声明本手教的考点核心词（`--kp "亚两面 单骑"`），在库内 `knowledge_point`
做子串粗筛，命中列出同考点题供人工复核取舍（只提醒、不自动否决）。
回测教训（2026-09-03）：4556 何切、3455 何切骨架都不撞库，但考点分别命中
L5_008 / L5_007——当初正是靠这道近亲闸拦下、未入库；L5_011 听牌升级骨架唯一、
考点无命中（可入库的干净例）。**出题规范：候选落库前两闸都过，近亲命中必人工复核。**
...
bank_version 单点 = `content/build/version.mjs`（当前 `2026.09.5-pilot`，换内容只 bump 那一处再重跑 roll-bank + publish）。
...
**2026-09-02 修订（用户裁定）：L1-L3 太简单 → 下限降为每级 ≥8**（纯切孤张的旧题
L1_001/L1_002/L2_001/L2_003 已删，库 70→66）；分布闸门同步为 L1-L3 ≥8、L4-L7 ≥10。
**2026-09-03 加 L5_011（听牌升级）→ 库 67**；同天再加 L3_011（四连形听牌）→ **库 68**
（分布 8/8/11/10/11/10/10）。
...
## 九、专项分类（`category` 字段，2026-09-03 定）

App「专项训练」Tab 的分类口径。**权威来源 = 词汇库大分类**
（vocabulary.md §二），App 侧把七个大分类归并成 5 组呈现；每题恰属一组，
落库为 `category` 字段（必填枚举，verify CLI 拦截缺失/未知值）——**不用
`knowledge_point` 关键词猜**（自由文本，猜出来的分类会随措辞漂移、且一题多投）。

| category | 专项名 | 大分类 | 装什么 | 现有题数 |
|---|---|---|---|---|
| `basic` | 搭子与进张 | 一 构成单位 + 三 进度与度量 | 面子/搭子/雀头辨认、进张张数口径、搭子价值对比 | 11 |
| `composite` | 复合形与长连 | 二 复合形/重叠块 | 帽子、中膨、亚两面、两嵌、四连五连、八连的读法与切法 | 18 |
| `structure` | 整手拆搭 | 五 整手组织 | 五搭子满员、对子过剩、拆搭顺序、二向听整序 | 20 |
| `tenpai` | 听牌与留法 | 四 听牌与待ち | 听牌形式优劣、留法二选一、听牌升级 | 8 |
| `strategy` | 取舍与不倒退 | 六 取舍规则 + 七 边界概念 | 不倒退原则、愚形让位、并列抉择、综合判断 | 11 |

**归类规则（出题时人工定，判据按序）**：

1. 考点的主角是**一个形状**（帽子/两嵌/五连……）→ `composite`；主角是**听牌后的形式**
   （两面 vs 单骑、留法选择）→ `tenpai`。
2. 考点的主角是**整手结构**（搭子数、对子数、块满不满、先切谁的整序）→ `structure`；
   是**跨形状的取舍原则**（不倒退、愚形让位、并列怎么看）→ `strategy`。
3. 只教「认出单位 / 算张数 / 比价值」的基础题 → `basic`。

**现存 68 题归属明细**（新题照此表追加；改题换考点须同步改 category）：

| 级 | category | 题号 |
|---|---|---|
| L1 | basic | 003（雀头识别）、005-008（搭子识别×4）、010（刻子跟班价值） |
| L1 | composite | 009（重张识别） |
| L1 | tenpai | 004（听牌张数对比） |
| L2 | basic | 002（进张口径）、005-007（嵌张/边张/两面识别）、008（进张对比） |
| L2 | structure | 004（保搭 vs 拆搭）、009（搭子排序超载） |
| L2 | composite | 010（长连余张） |
| L3 | composite | 001-011（全级：复合形与多面张） |
| L4 | composite | 001（五连切端） |
| L4 | structure | 002-010（满员/对子过剩/拆搭顺序/让位） |
| L5 | composite | 001、003、004、009（五连/长连切法） |
| L5 | tenpai | 002、005-008、010、011（听法与留法） |
| L6 | strategy | 001-009（不倒退×6、并列×2、愚形让位） |
| L6 | structure | 010（愚形一向听整序） |
| L7 | strategy | 001（并列最优）、002（综合判断） |
| L7 | structure | 003-010（对子过剩、整序×4、三色竞争、散搭取舍） |

> App 侧消费见 web-v1.md §一（`/special`、`/special/:cat`、`/drill`）；扩量到 200 题时
> 各组只增不减，某组长期 <8 题视为内容缺口（对齐起步批分布闸门的口径）。
